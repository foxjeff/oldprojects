...
class Basecamp
  ...
  class Record #:nodoc:
    ...
    def initialize(type, hash, xml_response)
      @type = type
      @hash = hash
  ...
  def companies project_id
    records "company", "/projects/#{project_id}/contacts/companies"
  end 
  ...
  # will raise 404 if attempting to retr non-existent comments for an archived project
  ...
  def request(path, parameters = {}, second_try = false)
    ...
    if response.code.to_i / 100 == 2
      ...
      [typecast_value(result), xml_response]
    elsif response.code == "302" && !second_try
      connect!(@url, !@use_ssl)
      request(path, parameters, true)
    else
      raise "#{response.message} (#{response.code}); for path: #{path}, #{parameters}"
    end
  end
  ...
  def record(path, parameters={})
    result, xml_response = request(path, parameters)
    (result && !result.empty?) ? Record.new(result.keys.first, result.values.first, xml_response) : nil
  end
  ...
  def records(node, path, parameters={})
    ...
    result.map { |row| Record.new(node, row, xml_response) }
  end
      ...
      when Hash
        if value.has_key?("__content__")
          ...
          value.empty? ? nil : value.inject({}) do |h,(k,v)|
            h[k] = typecast_value(v) unless k=="type"
            h
          end
        end
      when Array
...
#           val['type'] = "hash"
           YAML.object_maker( Basecamp::Record, val )
       end
...
scrip/console
	...
	b=Basecamp.new 'arl-amrita.projectpath.com', 'anandfox', 'pw'
	ps=b.projects
	ps.each {|p| puts "#{p['name']} was last changed #{p['last-changed-on'].class==Time ? distance_of_time_in_words_to_now(p['last-changed-on']) : "has never been changed" } ago"}
		Telematics was last changed 16 days ago
		Safelock was last changed 5 days ago
		ITEWS-PRMC was last changed 49 days ago
		Board Design was last changed 12 days ago
		Enterprise & Web was last changed 1 day ago
		System Tools & Libraries was last changed 15 days ago
		Firmware was last changed 17 days ago
		Research was last changed 32 days ago
		Cyber Security was last changed 1 day ago
